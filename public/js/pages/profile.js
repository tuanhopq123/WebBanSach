...
function showToast(msg, type = 'success') {
    ...
    container.style.cssText = 'position:fixed;top:20px;right:20px;z-index:1055;'; // high z-index to show above modals
  ...
  try{
      ...
      <div class="row">
          ...
          <div class="profile-header-card text-center d-flex flex-column align-items-center">
            ...
            <div class="bg-white bg-opacity-10 rounded-3 p-3 w-100 text-start">
              ...
              <div class="d-flex justify-content-between">
                <span class="opacity-75">Vai trò</span>
                <span class="fw-semibold bg-white text-primary px-2 rounded small">${roleName}</span>
              </div>
            </div>
            ${adminBtnHTML}
          </div>
        ...
        <div class="col-lg-8">
          <ul class="nav nav-pills mb-4">
            <li class="nav-item">
              <a class="nav-link active" href="#">📦 Lịch Sử Đơn Hàng</a>
            </li>
          </ul>
          <div id="orders-container">
            <div class="text-center text-muted py-4">Đang tải lịch sử đơn hàng...</div>
          </div>
        </div>
      </div>
    `;
    ...
    try {
      ...
      if (!orders || orders.length === 0) {
        ...
        const orderHtml = orders.map(ord => {
          const statusColors = {
          ...
          if (ord.items && ord.items.length > 0) {
             itemsHtml = ord.items.map(item => {
                ...
                if (ord.status === 'Delivered' && bId) {
                    if (existingReview) {
                        reviewBtn = `<button class="btn btn-outline-secondary btn-review" 
                          data-book-id="${bId}" 
                          data-order-id="${ord._id}"
                          data-review-id="${existingReview._id}"
                          data-rating="${existingReview.rating}"
                          data-comment="${existingReview.comment ? existingReview.comment.replace(/"/g, '&quot;') : ''}"
                          data-book-title="${bTitle.replace(/"/g, '&quot;')}" 
                          data-book-img="${bImg}">✏️ Sửa Đánh Giá</button>`;
                    } else {
                        reviewBtn = `<button class="btn btn-outline-primary btn-review" 
                          data-book-id="${bId}"
                          data-order-id="${ord._id}"
                          data-book-title="${bTitle.replace(/"/g, '&quot;')}" 
                          data-book-img="${bImg}">✍️ Đánh giá</button>`;
                    }
                }
                ...
                return `
                  <div class="item-card d-flex justify-content-between align-items-center">
                    <div class="d-flex align-items-center gap-3">
                        <img src="${bImg}" class="rounded shadow-sm" style="width:40px; height:56px; object-fit:cover;">
                        <div>
             ...
             itemsHtml = '<div class="text-muted small">Không có sản phẩm chi tiết</div>';
          }
            ...
            <div class="order-card">
              ...
              <div class="d-flex justify-content-between align-items-center pt-2 border-top">
                <div class="text-muted small">
                  Thanh toán: <span class="fw-semibold text-dark">${ord.paymentMethod}</span>
                </div>
                <div class="text-end">
                  <span class="text-muted small">Tổng thanh toán:</span>
                  <span class="fw-bold text-danger ms-2 fs-5">${currency(ord.totalAmount)}</span>
                </div>
              </div>
            </div>
          `;
      ...
      ordersContainer.innerHTML = `<div class="alert alert-danger shadow-sm">❌ Không tải được danh sách đơn hàng.</div>`;
    }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.body.addEventListener('click', (e) => {
        if (e.target.closest('.btn-review')) {
            ...
            const bookImg = btn.dataset.bookImg;
            ...
            document.getElementById('review-update-id').value = reviewId || '';
            
            document.getElementById('review-book-info').innerHTML = `
                <img src="${bookImg}" style="width:48px;height:64px;object-fit:cover;border-radius:6px;" class="shadow-sm">
                <div>
                    <div class="fw-semibold text-dark">${bookTitle}</div>
                    <div class="small text-muted">Đơn hàng: #${orderId}</div>
                </div>
            `;
            
            // clear or set form
            document.getElementById('review-rating').value = rating || "5";
            document.getElementById('review-comment').value = comment || "";
            document.getElementById('reviewModalLabel').textContent = reviewId ? "✏️ Cập nhật đánh giá" : "🌟 Đánh giá sản phẩm";
            document.querySelector('#profile-review-form button[type="submit"]').textContent = reviewId ? "CẬP NHẬT TRẢI NGHIỆM" : "GỬI ĐÁNH GIÁ";
    ...
    document.getElementById('profile-review-form').addEventListener('submit', async (e) => {
        ...
        try{
            ...
            // Rerender page to show "Sửa Đánh Giá"
